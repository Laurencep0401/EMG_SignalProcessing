...
% DATA PREPARATION
fs = fs_EMG; % Variable name (or value) of the sampling frequency of EMG signal (Hz)
S_raw = EMGraw; % Variable name of the signal vector of the EMG signal chanel of interest [in V]
t = tEMG; % Variable name of the time vector of EMG signal [in sec]
...
s_raw_PeakTresh = 1*10^-12; % Minimum height difference value between the peaks
...
cutoff_freq_range_BS = 1; % Range around the Hum Frequency to set the
        % cutoff frequencies (Hz). That means the cutoff frequencies will  
        % be (hum_freq_BS-cutoff_freq_range_BS) and
...
filtering_type_BS = 'bidirectional'; % The filtering process. 
        % Options: 'forward' or 'bidirectional'
...
cutoff_freq_BP = [30, 500]; % BP Cutoff frequencies (Hz)
...
filter_choice_BP = 'butter'; % BP Filter Design Method. Options:
        % 'butter' | 'cheby1' | 'cheby2' | 'cls' | 'ellip' | 'equiripple' |
        % 'freqsamp' | 'kaiserwin' | 'ls' | 'maxflat' | 'window'
filtering_type_BP = 'bidirectional'; % The filtering process. 
        % Options: 'forward' or 'bidirectional'
